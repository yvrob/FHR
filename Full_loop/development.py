...
def create_couple(path, isotopes, reactions, flux_spectrum, cs_og, elements):
    title='''=couple
            ...
            for r in range(len(reactions)):
                if int(reactions[r]) not in excluded:
                    if np.isnan(cs_og[i,r])==False:
                        array71+='{} '.format(isotope_to_code(isotopes[i],elements,'origen'))
...
=shell
dir
cp ft33f001 "${INPDIR}/origen.f33"
end
'''

    scale_input = block1 + block2 + block6 + endblock 
...
#%%
def create_origen(path_origen,pebble_number,path_fuel,elements,time_step, flux_spectrum, fuel_volume):
    intro='''=shell
...
'''
    ...
    mat='mat{\niso=[\n'
    for i in range(len(isotopes)):
        mat+='{}={}\n'.format(code_to_isotope(isotopes[i],elements), abs(quantities[i]*fuel_volume))
    mat+=']\nunits={}\n'.format('MOL' if all([q>=0 for q in quantities]) else 'GRAMS')
    mat+='}\n'
    timing='time{{\nt=[{}]\nunits = DAYS\n}}\n'.format(time_step)
...
'''
    ...
    origen_file=options+bounds+cases+end
    with open(path_origen, 'a') as file:
...
if __name__=='__main__':
    ...
    # Core 
    divider=3.70
    rad_core=105/divider # Core radius (cm)
...
#    scale_exe = 'C:/SCALE-6.2.3/bin/scalerte.exe'
    folder_scale = './SCALE_data/test/' # Folder where SCALE data will be stored, create it if does not exist
    ...
    t=time()
    # print('Calculating volume of fuel in each pebble')
    # nvol=1000000
    # subprocess.run([serpent_exe, path+'input_one_pebble', '-omp', str(omp), '-checkvolumes',str(nvol)], stdout=subprocess.PIPE, stderr=subprocess.PIPE) # Run SERPENT to determine fuel volume
    # with open('input_one_pebble.mvol') as f:
    #     for line in f:
    #         if 'fuel_1' in line:
    #             values_line=line.strip().split(' ')
    #             fuel_volume=float(values_line[-3])
    #             print('Fuel volume per pebble: {:.7} cm^3'.format(fuel_volume))
    fuel_volume=0.1554
    m_hm_ini=(m_U5+m_U8)*fuel_volume
    # print('Initial heavy metal mass per pebble: {:.7} g'.format(m_hm_ini))
    # print('\tDONE. Time elapsed: {:.4} s'.format(time()-t))   
    ...
    while max(bu)<=100:
        ...
        print('Collapsing cross section for each pebble and creating SCALE input...')
        cs_og=np.transpose(np.matmul(cs_table,det.tallies)/sum(det.tallies),(2,0,1))
        flux_spectrum=det.tallies.T
        break
        with open(path_fuel+'_'+str(ind),'w'):
            pass
        t=time()
        for i_pebble in range(n_pebbles):
            create_couple(path_scale+str(i_pebble)+'.inp', isotopes, reactions, flux_spectrum[i_pebble], cs_og[i_pebble],elements) # Create a COUPLE input
            create_origen(path_scale+str(i_pebble)+'.inp',i_pebble+1,path_fuel,elements,dt,flux_spectrum[i_pebble],fuel_volume) # Create an ORIGEN input
        print('\tDONE. Time elapsed: {:.4} s'.format(time()-t))    
        
        from itertools import repeat
        def create_scale(path_scale,i_pebble,isotopes,reactions,flux_spectrum,cs_og,elements,fuel_volume):
            create_couple(path_scale+str(i_pebble)+'.inp', isotopes, reactions, flux_spectrum[i_pebble], cs_og[i_pebble],elements) # Create a COUPLE input
            create_origen(path_scale+str(i_pebble)+'.inp',i_pebble+1,path_fuel,elements,dt,flux_spectrum[i_pebble],fuel_volume) # Create an ORIGEN input
        ...
        pool_outputs = pool.starmap(create_scale,zip(repeat(path_scale),range(n_pebbles),repeat(isotopes),repeat(reactions),np.nditer(flux_spectrum),np.nditer(flux_spectrum),repeat(elements),repeat(fuel_volume)))
        pool.close() # no more tasks
        pool.join()  # wrap up current tasks
        break
    
        print('Running SCALE and create Serpent input')
        t=time()
        subprocess.run([scale_exe, '-I', str(omp),'-m', path_scale+'.inp'], stdout=subprocess.PIPE, stderr=subprocess.PIPE) # Run COUPLE
        ...
        create_serpent_block(path_fuel+'_'+str(ind),path_scale+'.out',temp,fuel_dens) # Create new Serpent input
        os.system("cp -rf {} {}".format(path_fuel+'_'+str(ind),path_fuel))
        print('\tDONE. Time elapsed: {:.4} s'.format(time()-t))
       # t=time()
        #subprocess.run([serpent_exe, path_serpent+input_serpent, '-omp', str(omp)], stdout=sss_log, stderr=subprocess.PIPE) # Run ORIGEN
        #print('\tDONE. Time elapsed: {:.4} s'.format(time()-t))    
