...
if temperature%300!=0:
    ...
    if energy_structure!='':
        ...
        for i in range(8):        
            f_detectors.write('det {} dm fuel_{} de E\n'.format(i+1,i+1))
    else:
        for i in range(8):
            f_detectors.write('det {} dm fuel_{}\n'.format(i+1,i+1))
    for r in range(len(reactions)):
        ...
        for i in range(len(isotopes)):
            ...
            if excluded==False:
                f_detectors.write('% {}\n'.format(isotopes_simple_name[i]))
                for j in range(8):                  
                    if energy_structure!='':
                         f_detectors.write("det\t{:<1}_{:1}\tde\tE\tdm\tfuel_{:1}\tdr\t{:<1}\t{:<1}\tdt\t3\t{}\n".format(isotopes_simple_name[i]+'_'+"%03d"%(reactions[r]),j+1,j+1,reactions[r],isotopes_simple_name[i],j+1))
                    else:
                         f_detectors.write("det\t{:<1}_{:1}\tdm\tfuel_{:1}\tdr\t{:<1}\t{:<1}\tdt\t3\t{}\n".format(isotopes_simple_name[i]+'_'+"%03d" %(reactions[r]),j+1,j+1,reactions[r],isotopes_simple_name[i],j+1))
    print('\nDONE')
