    ...
    with open(path+"tmp", 'r') as istr:
        with open(path+'fpb_pos', 'w') as ostr:
            ...
            for line in istr:
                ...
                index+=1    
def write_geometry(path,plot,qual,delta_x,delta_z,zmin,zmax,pebble_rad,rad_core,refl_thickness=0,bc=0,shape='cylinder'):
    geometry_file=open(path+'geometry','w')
    geometry_file.write('set bc {}\n'.format(bc))
    if plot==True:   
        string='''
        %%---plotting geometry
        plot 1 {} {}
...
if __name__=='__main__':
    ...
    write_geometry(path,plot,qual,delta_x,delta_z,zmin,zmax,pebble_rad,rad_core,refl_thickness,bc, shape) # Write geometry file   
