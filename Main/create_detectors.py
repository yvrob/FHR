    ...
    for r in range(len(reactions)):
        ...
        for i in range(len(isotopes)):
            ...
            if excluded==False:
                ...
                if energy_structure!='':
                    f_detectors.write("det\t{:<1}\tde\tE\tdm\tfuel\tdr\t{:<1}\t{:<1}\tdt\t3\t{}\n".format(isotopes_simple_name[i]+'_'+"%03d"%(reactions[r]),reactions[r],isotopes_simple_name[i],1))
                else:
                    f_detectors.write("det\t{:<1}\tdm\tfuel\tdr\t{:<1}\t{:<1}\tdt\t3\t{}\n".format(isotopes_simple_name[i]+'_'+"%03d" %(reactions[r]),reactions[r],isotopes_simple_name[i],1))
    print('\nDONE')
